"""
GSS Orion V3 — Mission I/O & Workspace Manager.
Handles directory setup, project manifest updates, and result saving.
(Extracted from persistence.py to satisfy SRP compliance).
"""
...
def initialize_mission_environment(mission_data: dict):
    ...
    try:
                    ...
                    for agent in selected_agents:
                        ...
                        agent_specs = {
                            **specs,
                            ...
                            "constraints": list(
                                set((specs.get("constraints") or []) + (agent_skill.get("constraints") or []))
                            ),
        ...
        if updated:
            projects_file.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
            event_bus.emit("PERSISTENCE", "EnvironmentReady", "OK", f"Project {project_id} updated.")
